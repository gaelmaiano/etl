...
def load_to_mysql(df, table_name, engine, index_as_pk=False, if_exists_mode='append'):
...
def main():
    ...
    if 'region' in sqlite_data:
        ...
        load_to_mysql(df, 'Regions', engine)
    if 'revendeur' in sqlite_data:
        ...
        load_to_mysql(df, 'Revendeurs', engine)
    if 'produit' in sqlite_data:
        ...
        load_to_mysql(df, 'Produits', engine)
    if 'production' in sqlite_data:
        ...
        load_to_mysql(df, 'Productions', engine, index_as_pk=True)

    # Commandes et lignes de commande depuis CSV
    ...
    load_to_mysql(commandes, 'Commandes', engine)
    ...
    load_to_mysql(lignes, 'LignesCommande', engine, index_as_pk=True)
