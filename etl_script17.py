...
def load_to_mysql(df, table_name, engine, index_as_pk=False, if_exists_mode='replace'):
    ...
    for col in df.columns:
        if df[col].dtype == 'object':
            ...
            # Si la colonne est l'index du DataFrame ET index_as_pk est True (ce qui signifie qu'it will be the PK)
...
def main():
    ...
    try:
        ...
        if not df_produits.empty:
            # Rename 'product_id' to 'produit_id' to match expected FK in LignesCommande
            ...
            # Ensure 'produit_id' is a column for loading
            if df_produits.index.name == 'produit_id':
                df_produits = df_produits.reset_index()

            df_produits['produit_id'] = df_produits['produit_id'].astype(int) # Ensure type is int
        ...
        if not df_commandes.empty:
            ...
            # Ensure 'commande_id' is a column for loading (if it was an index, reset it)
            if df_commandes.index.name == 'commande_id':
                 df_commandes = df_commandes.reset_index()
            
            # Ensure types are correct before loading
            df_commandes['commande_id'] = df_commandes['commande_id'].astype(